...
void main()
{
	...
	texCoord.y = 1.0 - uv.y;
	mediump vec3 Albedo = texture2D(diffuseTex, texCoord).rgb;
	
	mediump vec3 diffuse = diffuseColor * max(dot(normalInModel_normalized, lightDirInModel_normalized), 0.0);
